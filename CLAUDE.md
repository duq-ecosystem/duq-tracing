# ⛔ ЗАПРЕТ ЛОКАЛЬНОГО E2E ТЕСТИРОВАНИЯ

## КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО ЛОКАЛЬНО:
- docker compose up / docker run
- uvicorn / fastapi / go run / любые серверы
- создание локальных БД (postgres, redis)
- e2e тесты на локальной инфре

## РАЗРЕШЕНО ЛОКАЛЬНО:
- pytest unit/integration (mocks/fakes)
- go test (mocks)
- uv sync / go build для зависимостей
- make verify

## E2E ТЕСТИРОВАНИЕ ТОЛЬКО НА ПРОД VPS
